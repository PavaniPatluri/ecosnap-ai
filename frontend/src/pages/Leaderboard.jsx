  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="mt-4 text-gray-600 dark:text-gray-400 text-lg">Top users making the biggest impact.</p>
      </div>

      <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700 px-8 py-4 grid grid-cols-12 gap-4 font-semibold text-gray-500 dark:text-gray-400 uppercase text-sm tracking-wider">
        ...
        <div className="divide-y divide-gray-50">
          {leaders.map((user, index) => (
            <div key={user.id} className="px-8 py-6 grid grid-cols-12 gap-4 items-center hover:bg-gray-50 dark:bg-gray-900 transition-colors">
              ...
              <div className="col-span-7 flex items-center space-x-4">
                <div className={`h-12 w-12 rounded-full flex items-center justify-center font-bold text-lg
                  ${index === 0 ? 'bg-yellow-100 text-yellow-700 border-2 border-yellow-200' : 
                    index === 1 ? 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-2 border-gray-200 dark:border-gray-700' :
                    index === 2 ? 'bg-amber-50 text-amber-700 border-2 border-amber-200' :
                    'bg-eco-50 text-eco-700 border border-eco-100'}`}
                >
                  {user.username.charAt(0).toUpperCase()}
                </div>
                <span className={`text-lg ${index < 3 ? 'font-bold text-gray-900 dark:text-white' : 'font-medium text-gray-700 dark:text-gray-300'}`}>
                  {user.username}
                </span>
              </div>
              <div className="col-span-3 text-right">
                <span className={`text-2xl font-black ${index === 0 ? 'text-yellow-500' : 'text-gray-900 dark:text-white'}`}>
                  {user.eco_points}
                </span>
              </div>
            </div>
